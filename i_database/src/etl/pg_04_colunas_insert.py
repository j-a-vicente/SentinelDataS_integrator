    ...
    with open('src/scriptsExtracao/Sentinel_list_instacia_database.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                with open('src/scriptsExtracao/pg_04_insert_colunas.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
                ...
                if success:
                    for row_r in resutRemoto:       
                        ...
                        if util.if_null(str(row_r[1])) != 0 :
                            ...
                            insert_v = insert_v + "\n"+ F"('{str(o_row[7])}','{str(row_r[0])}','{str(row_r[1])}','{str(row_r[2])}','{str(row_r[3])}','{str(row_r[4])}','{str(row_r[5])}'),"
