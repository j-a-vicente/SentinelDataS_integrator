...
def extracao():
    ...
    with open('src/scriptsExtracao/Sentinel_list_instacia_database.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                with open('src/scriptsExtracao/sql_03_insert_tabela.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
                ...
                if success:
                    for row_r in resutRemoto:       
                        ...
                        if util.if_null(str(row_r[1])) != 0 :
                            #print(str(o_row[7]),str(row_r[0]),str(row_r[1])) #,str(row_r[2]),str(row_r[3]),str(row_r[4]),str(row_r[5]),str(row_r[6]),str(row_r[7]),str(row_r[8]),str(row_r[9])      )          
   
                            insert_v = insert_v + "\n"+ F"('{str(o_row[7])}','{util.remover_aspas(str(row_r[0]))}','{str(row_r[1])} '),"
    ...
    try:
        ...
        o_result = sp_insert_executor.execute_command('CALL sgbd.sp_pg_insert_bdtabela();')
    except Exception as e:
        print(f"Error: {e}")
