...
def extracao():
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                # Abrir conexão remota com o servidor de banco, de onde será extraido os METADADOS.
                    origem = DatabaseConnection("Oracle", srv_remot, "NaoEnecessario", "dcdados", port_remot )
                ...
                with open('src/scriptsExtracao/ocl_01_insert_database.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:                     
                        ...
                        insert_v = insert_v + "\n"+ F"('{str(o_row[0])}','{str(row_r[0])}','{str(row_r[2])}'),"
    ...
    try:
        ...
        o_result = sp_insert_executor.execute_command('CALL sgbd.sp_pg_insert_database();')
    except Exception as e:
        print(f"Error: {e}")    
