...
def gravar(insert_v):
    ...
    # Iniciar os recordSet com a conexão de destino.      
      ...
      insert_query = """ INSERT INTO stage.tbindex(idbasededados, schemaname, tablename, index_name, index_type) VALUES """ 
...
def extracao():
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                # Abrir conexão remota com o servidor de banco, de onde será extraido os METADADOS.
                    origem = DatabaseConnection("SQLServer", srv_remot, database_remot, "dcdados", port_remot )
                ...
                with open('src/scriptsExtracao/sql_05_insert_index.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
                ...
                if success:
                    for row_r in resutRemoto:       
                        ...
                        #print(str(o_row[7]),str(row_r[0]),str(row_r[1]),str(row_r[2]),str(row_r[3])) #,str(row_r[2]),str(row_r[3]),str(row_r[4]),str(row_r[5]),str(row_r[6]),str(row_r[7]),str(row_r[8]),str(row_r[9])      )          
                        if util.if_null(str(row_r[1])) != 0 :
                            insert_v = insert_v + "\n"+ F"('{str(o_row[7])}','{str(row_r[0])}','{str(row_r[1])}','{util.remover_aspas(str(row_r[2]))}','{util.remover_aspas(str(row_r[3]))}'),"
    ...
    try:
        ...
        o_result = sp_insert_executor.execute_command('CALL sgbd.sp_pg_insert_tbindex();')
    except Exception as e:
        print(f"Error: {e}")
