...
# Recupera o local dos arquivos de extração do arquivo .env
...
def gravar(insert_v):
    ...
    # Iniciar os recordSet com a conexão de destino.      
      ...
      insert_query = """ INSERT INTO stage.instancia_pg(idinstancia, name, setting, unit, category) VALUES """ 
...
def extracao():
    ...
    with open(DIR_EXT+'Sentinel_list_instacia.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                with open(DIR_EXT+'pg_00_instancia_setting.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:                     
                        ...
                        insert_v = insert_v + "\n"+ F"('{str(o_row[0])}','{str(row_r[0])}','{str(row_r[1])}','{str(row_r[2])}','{str(row_r[3])}'),"
    ...
    try:
        ...
        o_result = sp_insert_executor.execute_command('CALL sgbd.sp_insert_instancia_pg();')
    except Exception as e:
        print(f"Error: {e}")
...
if __name__ == "__main__":
    ...
    load_dotenv()
    DIR_EXT = os.getenv("I_DATABASE_D_EXT")
    print(DIR_EXT)
