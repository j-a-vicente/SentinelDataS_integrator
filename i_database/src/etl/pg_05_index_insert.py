...
def gravar(insert_v):
    ...
    # Iniciar os recordSet com a conexão de destino.      
      ...
      insert_query = """ INSERT INTO stage.tbindex(idbasededados, schemaname, tablename, index_name) VALUES """ 
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                # Abrir conexão remota com o servidor de banco, de onde será extraido os METADADOS.
                    origem = DatabaseConnection("PostgreSQL", srv_remot, database_remot, "dcdados", port_remot )
                ...
                with open('src/scriptsExtracao/pg_05_insert_index.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
                ...
                if success:
                    for row_r in resutRemoto:       
                        ...
                        if util.if_null(str(row_r[1])) != 0 :
                            insert_v = insert_v + "\n"+ F"('{str(o_row[7])}','{str(row_r[0])}','{str(row_r[1])}','{str(row_r[2])}'),"
