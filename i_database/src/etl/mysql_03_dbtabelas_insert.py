...
class mysql_01_insert_tabelas:
    ...
    def gravarLog(self):
        ...
        insert_query = insert_query + "\n"+ F"('{str(self.idlot)}','mddados','mysql_01_insert_tabelas','{util.remover_aspas(self.execEtapa)}','{str(self.total_registro_insert)}','{self.dhinicial}','{self.dhfinal}'  )" +';'				
    ...
    def gravar(self,insert_v):
        ...
        # Iniciar os recordSet com a conexão de destino.      
        insert_executor  = CommandExecutor(i_destino.connection)
        insert_query = """ INSERT INTO stage.bdtabela(idbasededados, schema_name, table_name) VALUES """ 
        insert_query = insert_query + "\n"+ insert_v +';'
        result = insert_executor.execute_command(insert_query)  
    ...
    def extracao(self):
        ...
        with open(F'{self.dir_dn}Sentinel_list_instacia_database.sql', 'r') as arquivo_sql:
            ...
            if success:
                for o_row in o_result: 
                    ...
                    with open(F'{self.dir_dn}mysql_03_insert_tabela.sql', 'r') as arquivo_sql:
                        o_query = arquivo_sql.read()
                    ...
                    if success:
                        for row_r in resutRemoto:        
                            ...
                            insert_v = insert_v + "\n"+ F"('{str(o_row[7])}','{str(row_r[0])}','{str(row_r[1])}'),"
...
if __name__ == "__main__":
    ...
    mysql_01_insert_tabelas_return = mysql_01_insert_tabelas(idlog_etl)
    print(mysql_01_insert_tabelas_return)    