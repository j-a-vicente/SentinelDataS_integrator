...
def gravar(insert_v):
    ...
    # Iniciar os recordSet com a conexão de destino.      
      ...
      insert_query = """ INSERT INTO stage.logins_instancia(idinstancia, loginname, acessement,tipo_login)  VALUES """ 
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:   
                        ...
                        query_json = F"""SELECT L.loginname AS 'loginname',L.isntname AS 'isntname',L.sysadmin AS 'sysadmin',L.securityadmin AS 'securityadmin',L.serveradmin AS 'serveradmin',L.setupadmin AS 'setupadmin',
       L.processadmin AS 'processadmin',L.diskadmin AS 'diskadmin',L.dbcreator AS 'dbcreator',L.bulkadmin AS 'bulkadmin'
FROM [sys].syslogins AS L WHERE L.loginname = '{str(row_r[0])}'  FOR XML PATH(''), ROOT('root')"""
                    ...
                    # Iniciar a leitura da consulta.
                        if success:
                            for row_j in resutRemoto_j:  
                                ...
                                insert_v = insert_v + "\n"+ F"('{str(o_row[0])}','{str(row_r[0])}','{json_data}','login'),"
    ...
    o_result = sp_insert_executor.execute_command('CALL sgbd.sp_insert_logins_instancia();')            
