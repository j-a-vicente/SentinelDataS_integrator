    ...
    with open('src/scriptsExtracao/Sentinel_list_instacia.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                srv_remot  = str(o_row[7]) # Servidor remoto
                ...
                with open('src/scriptsExtracao/sql_02_insert_tamanho_database.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:                     
                        ...
                        insert_v =  F"('{str(o_row[0])}','{util.remover_aspas(str(row_r[0]))}','{str(row_r[1])}')"
                        gravar(insert_v)       
                        #print(insert_v)   
                        insert_v = ''
