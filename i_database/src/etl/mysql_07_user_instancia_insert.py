    ...
    with open('src/scriptsExtracao/Sentinel_list_instacia.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                with open('src/scriptsExtracao/mysql_06_insert_user_login_srv.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:                     
                        ...
                        #print(str(o_row[0]),str(row_r[0]),str(row_r[2]))#,str(row_r[2]),str(row_r[3]),str(row_r[4]),str(row_r[5]),str(row_r[6]),str(row_r[7]),str(row_r[8]),str(row_r[9])      )          

                        json = util.trocar_aspas_aspasduplas(str(row_r[1])).replace('""','"')

                        insert_v = insert_v + "\n"+ F"('{str(o_row[0])}','{str(row_r[0])}','{json}','login'),"
