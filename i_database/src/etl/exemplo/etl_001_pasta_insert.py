		...
		# Carrega as variáveis com os valores dos servidores que serão conectados.
			...
			if success:
				...
				for o_row in o_result:    	
					...
					# Verifica se o valor extraido já existe no servidor de destino.
					r_query = "SELECT count(*) FROM [objects].[Pasta] WHERE [idEstancia]="+str(o_row[0])+" AND [ItemID]='"+str(o_row[2])+"' AND [ParentID]='"+str(o_row[3])+"' "
					
					# Executa a consulta no servidor de destino.					
					cursorSelect, success = i_query_executor.execute_query(r_query)

					# Se o valor do retorno for ZERO significa que o registro não existe e será gravado na sequência.
					if cursorSelect[0][-1] == 0:
						insert_query = """INSERT INTO [objects].[Pasta] ([idEstancia],[Nivel],[ItemID],[ParentID],[Localizacao],[Pasta],[Tipo],[DataDaCriacao],[DataDaModificacao],[DiasSemAlteracao] ) VALUES """ 
						...
						update_query = "UPDATE [objects].[Pasta] SET [DataDaModificacao] = '"+str(o_row[8])+"',[DiasSemAlteracao] = "+str(o_row[9])+" WHERE [idEstancia] = "+str(o_row[0])+" AND [ItemID] ='"+str(o_row[2])+"'"
