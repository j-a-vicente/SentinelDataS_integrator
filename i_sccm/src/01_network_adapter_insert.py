...
truncatQuery = 'truncate table stage.network_adapter'
...
with open('i_sccm/scriptExtracao/01_network_adapter.sql', 'r') as arquivo_sql:
...
if success:
    ...
    for o_row in o_result:   
      ...
      insert_query = """INSERT INTO stage.network_adapter(resourceid, adaptertype0, productname0, macaddress0, dhcpenabled0, dhcpserver0, dnsdomain0, dnshostname0, ipaddress0, deviceid0, ipenabled0, ipsubnet0, servicename0) VALUES """ 
      insert_v = insert_query + "\n"+ F"('{o_row[0]}','{o_row[1]}','{o_row[2]}','{o_row[3]}','{o_row[4]}','{o_row[5]}','{o_row[6]}','{o_row[7]}','{o_row[8]}','{util.if_null(o_row[9])}','{o_row[10]}','{o_row[11]}','{o_row[12]}');"
