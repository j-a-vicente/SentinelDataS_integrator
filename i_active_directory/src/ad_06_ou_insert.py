"""
Nesta está do mudolo Active Directory, os itens extraidos será do objeto [ou]
...
class ad_ou:
    ...
    # 6 - Inicia o processo de extração:
        self.total_registro_insert = self.listar_ou()
    ...
    def gravarLog(self):
        #print(F"Estapa: ad_ou, LogPai:{str(self.idlot)}, Total de registro:{str(self.total_registro_insert)}, dhInicial:{self.dhinicial}, dhFinal: {self.dhfinal}, Resultado: {self.execEtapa} ")
        ...
        insert_query = insert_query + "\n"+ F"('{str(self.idlot)}','ActiveDirectory','ad_ou','{self.execEtapa}','{str(self.total_registro_insert)}','{self.dhinicial}','{self.dhfinal}'  )" +';'				
    ...
    def gravar(self, insert_v):
        ...
        # Monta o valores de inserts.
        insert_query = """INSERT INTO stage.ad_ou(cont, name, objectcategory, objectclass, distinguishedname, whencreated, whenchanged) VALUES """ 
    ...
    def listar_ou(self):
        ...
        truncatQuery = 'truncate table stage.ad_ou'
    ...
    # Variáveis:
        ...
        try:        
            ...
            attributes=['name', 'objectCategory', 'objectClass', 'distinguishedName', 'whencreated', 'whenchanged']
            ...
            while True:                
                for entry in connection.entries:
                    ...
                    insert_v = insert_v + "\n"+  F"('{cont}','{util.remover_aspas(entry.name.value)}','{util.remover_aspas(entry.objectCategory.value)}','organizationalUnit','{util.remover_aspas(entry.distinguishedname.value)}','{util.whenC_to_datetime(entry.whenCreated.value)}','{util.whenC_to_datetime(entry.whenChanged.value)}'),"
