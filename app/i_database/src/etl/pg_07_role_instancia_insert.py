...
def gravar(insert_v):
    ...
    # Iniciar os recordSet com a conexão de destino.      
      ...
      insert_query = """ INSERT INTO stage.logins_instancia(idinstancia, loginname, acessement) VALUES """ 
    ...
    with open('src/scriptsExtracao/Sentinel_list_instacia.sql', 'r') as arquivo_sql:
        ...
        if success:
            ...
            for o_row in o_result: 
                ...
                with open('src/scriptsExtracao/pg_09_insert_role_group_.sql', 'r') as arquivo_sql:
                    o_query = arquivo_sql.read()
            ...
            # Iniciar a leitura da consulta.
                if success:
                    for row_r in resutRemoto:                     
                        ...
                        insert_v = insert_v + "\n"+ F"('{str(o_row[0])}','{str(row_r[0])}','{json}'),"
