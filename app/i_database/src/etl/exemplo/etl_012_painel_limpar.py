...
# Execute a consulta e obtenha o resultado e o status de sucesso
	...
	insert_query = 'TRUNCATE TABLE [stage].[Painel] '
...
# Iniciar a leitura da consulta.
				...
				for o_row in o_result:    	
					...
					if cursorSelect[0][-1] != 0 :			
						# 2 - verifica se o painel foi cadastrado.
								insert_query = """INSERT INTO [stage].[Painel] ([idPasta],[ParentID],[ItemID],[Localizacao],[Objeto],[Tipo],[DataDaCriacao],[DataDaModificacao],[DiasSemAlteracao],[Tamanho],[CreatedByUserName],[ModifiedByUserName]) VALUES """
								insert_query = insert_query + "\n"+"("+str(cursorSelect[0][-1])+",'"+str(o_row[0])+"','"+str(o_row[1])+"','"+str(o_row[2])+"','"+str(o_row[3])+"','"+str(o_row[4])+"','"+str(o_row[5])+"','"+str(o_row[6])+"',"+str(o_row[7])+",'"+str(o_row[8])+"','"+str(o_row[9])+"','"+str(o_row[10])+"');"	
								result = insert_executor.execute_command(insert_query)    									
		...
		print("Falha na consulta")
	# Executa a procedure que desativa os paineis que foram deletadas.
	insert_query = 'EXECUTE [stage].[SP_clear_painel] '
	result = insert_executor.execute_command(insert_query)    

	# Executa a procedure que cadastra o tamanho do painel na tabelas painelTamanho se o tamanho do painel mudou
	insert_query = 'EXECUTE [stage].[SP_painel_Tamanho_insert] '
	result = insert_executor.execute_command(insert_query)    
