"""
Nesta está do mudolo Active Directory, os itens extraidos será do objeto [Contact]
...
class ad_contact:
    ...
    # 6 - Inicia o processo de extração:
        self.total_registro_insert = self.listar_contact()
    ...
    def gravarLog(self):
        #print(F"Estapa: ad_contact, LogPai:{str(self.idlot)}, Total de registro:{str(self.total_registro_insert)}, dhInicial:{self.dhinicial}, dhFinal: {self.dhfinal}, Resultado: {self.execEtapa} ")
        ...
        insert_query = insert_query + "\n"+ F"('{str(self.idlot)}','ActiveDirectory','ad_contact','{self.execEtapa}','{str(self.total_registro_insert)}','{self.dhinicial}','{self.dhfinal}'  )" +';'				
    ...
    def gravar(self, insert_v):
        ...
        # Monta o valores de inserts.
        insert_query = """INSERT INTO stage.ad_contact(cont, name, displayname, mailnickname, mail, memberof, distinguishedname, objectcategory, objectclass, whencreated, whenchanged) VALUES """ 
    ...
    def listar_contact(self):
        ...
        truncatQuery = 'truncate table stage.ad_contact'
    ...
    # Variáveis:
        ...
        try:        
            ...
            search_filter = '(&(objectCategory=Person)(objectClass=contact))'

            # Atributos que seram extraidos.
            attributes=['name','displayName','mailNickname','mail','memberOf','distinguishedName','objectCategory','objectClass','whenCreated','whenChanged']
            ...
            while True:                
                for entry in connection.entries:
                    ...
                    insert_v = insert_v + "\n"+  F"('{cont}','{util.remover_aspas(entry.name.value)}','{util.remover_aspas(entry.displayName.value)}','{util.remover_aspas(entry.mailNickname.value)}','{util.remover_aspas(entry.mail.value)}','{util.remover_aspas(entry.memberOf.value)}','{util.remover_aspas(entry.distinguishedName.value)}','{entry.objectCategory.value}','contact','{util.whenC_to_datetime(entry.whenCreated.value)}','{util.whenC_to_datetime(entry.whenChanged.value)}'),"
...
if __name__ == "__main__":
    ...
    ad_contact_return = ad_contact(idlog_etl)
    print(ad_contact_return)
